...
bool checkBalance(const char *source, int n)
{
    ...
    if (IsStackEmpty(s) && (closing - opening) == 0)
    {
        FreeStack(s);
        return true;
    }
...
bool XMLDocument_load(XMLDocument *doc, const char *path)
{
    ...
    // check consistency
    if (checkBalance(source, size - newlines))
    {
...
static void Format(FILE *file, XMLNode *node, int indent, int times)
{
    for (int i = 0; i < node->children.size; i++)
    {
        ...
        else
        {
            ...
            if (child->children.size == 0)
            {
                if (strlen(child->inner_text) > 200) // if inner text size is large make new line
                {
...
bool XMLDocument_write(XMLDocument *doc, const char *path, const char choice)
{
    ...
    if (choice == 'f')
    {
        Format(file, doc->root, 4, 0); // identation = 3 spaces (can be changed)
    }
    else if (choice == 'm')
    {
...
int get_Follower_id(XMLNode *user, int index)
{
    XMLNode *follower = XMLNode_child(user, 3);
    XMLNodeList *followers = XMLNode_children(follower, "follower");
